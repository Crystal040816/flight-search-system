...
截至 2026-07-23，本项目 ODS、DWD、DWS 和 ADS 数据链路已完成。DWD、DWS 的 2026-07-21 基线验收全部通过；MySQL ADS 保持 4 张服务表，2026-07-23 时间字段与起飞时刻粒度升级已发布，10 项发布后质量指标和多起飞时刻业务抽查均通过。
...
| ADS | `ads_route_cabin_lowest_price` | 714,982 |
...
### 4.3 ADS 2026-07-23 时间粒度发布验收

两张最低价表新增首段起飞、末段到达和整条行程时长字段；`ads_route_cabin_lowest_price` 的粒度调整为“搜索日 + 航线 + 出发日 + 起飞 Epoch + 舱型”。正式发布后，最低价表为 26,562 行，舱型最低价表为 714,982 行，航线排行和航司占比分别保持 1,246 行和 71 行。

`ads_quality_checks.sql` 的 10 项检查全部通过：价格、座位、出发地、目的地、舱型、时间字段和主键粒度异常数均为 0，总体最低价报价均能在舱型表中找到。业务抽查选择 `2022-04-19 / LAX -> BOS / 2022-04-26`，按每个起飞 Epoch 选择最低价后返回 67 行，67 个起飞时刻全部唯一。
...
- 出发地和目的地机场代码、城市、国家；
- 出发日期、最低含税总价、平均价和币种；
- 首段当地起飞时间、末段当地到达时间和整条行程时长；
- 首段航司、剩余座位；
- 舱型类别、逐航段舱型、混合舱型标识；
- 逐航段机型描述；
- 同一航线、日期与起飞时刻下按舱型比较最低价，并支持每个起飞时刻选择最低价。
...
当前 DWD/DWS 基线和 2026-07-23 ADS 发布状态为 `ACCEPTED/FROZEN`。后续变更必须先备份、执行 dry-run、使用新的验收标识并重新核对对应层行数。不得覆盖旧质量记录或用旧验收结果证明新数据有效。
